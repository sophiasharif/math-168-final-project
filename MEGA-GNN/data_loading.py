...
def get_data(args, data_config):
    ...
    motif_cols = [c for c in df_edges.columns.tolist() if c.startswith('motif_')]
    if motif_cols:
        edge_features = edge_features + motif_cols
        logging.info(f'Using motif feature columns: {motif_cols}')
