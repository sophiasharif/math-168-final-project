...
class MultiMPNN(torch.nn.Module):
    def __init__(self, num_features, num_gnn_layers, n_classes=2, n_hidden=100, 
        ...
        self.use_motif_encoder = bool(getattr(args, "motif_encoder", False))
        self.motif_dim = int(getattr(args, "motif_dim", 0))
        self.base_edge_dim = int(edge_dim - self.motif_dim) if edge_dim is not None else 0
        if self.use_motif_encoder and self.motif_dim > 0 and self.base_edge_dim > 0:
            self.edge_emb = nn.Linear(self.base_edge_dim, n_hidden)
            self.edge_motif_emb = nn.Linear(self.motif_dim, n_hidden)
            if args.reverse_mp or args.reverse_mp_lp:
                self.edge_emb_rev = nn.Linear(self.base_edge_dim, n_hidden)
                self.edge_motif_emb_rev = nn.Linear(self.motif_dim, n_hidden)
        else:
            self.use_motif_encoder = False
    ...
    def _encode_edge_attr(self, edge_attr, reverse=False):
        if self.use_motif_encoder:
            base_attr = edge_attr[:, :self.base_edge_dim]
            motif_attr = edge_attr[:, self.base_edge_dim:self.base_edge_dim + self.motif_dim]
            base_emb = self.edge_emb_rev(base_attr) if reverse else self.edge_emb(base_attr)
            motif_emb = self.edge_motif_emb_rev(motif_attr) if reverse else self.edge_motif_emb(motif_attr)
            return base_emb + motif_emb
        return self.edge_emb_rev(edge_attr) if reverse else self.edge_emb(edge_attr)
    ...
    def forward(self, data):
        if isinstance(data, HeteroData):
            ...
            edge_attr_dict = {
                    ("node", 'to', 'node'): self._encode_edge_attr(data['node', 'to', 'node'].edge_attr, reverse=False), 
                    ("node", 'rev_to', 'node'): self._encode_edge_attr(data['node', 'rev_to', 'node'].edge_attr, reverse=True),    
                }
            ...
            edge_attr = self._encode_edge_attr(data.edge_attr, reverse=False)
            ...
            if self.args.task == 'edge_class':
                ...
                if self.args.ports_batch:
                    ...
                    neg_edge_attr = self._encode_edge_attr(data.neg_edge_attr, reverse=False) 
                    pos_edge_attr = self._encode_edge_attr(data.pos_edge_attr, reverse=False)
